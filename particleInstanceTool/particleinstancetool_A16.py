...
#countGetValue
    def clickButtin_createInstance_multiThread(self):  #test maya multi thread
       ...
       # for i in range(0,jobSplitNum):
        ...
        maya.utils.executeInMainThreadWithResult(self.multiJob_00)       
        maya.utils.executeInMainThreadWithResult(self.multiJob_01)       
        maya.utils.executeInMainThreadWithResult(self.multiJob_02)       
        maya.utils.executeInMainThreadWithResult(self.multiJob_03)       
        maya.utils.executeInMainThreadWithResult(self.multiJob_04)       
    ...
    def doGetDataFromParticles(self):
        ...
        #multi thread start
    def multiJob_00(self):
        for self.n in self.jobNumDict[0]:
            self.perParticle = self.selectParticleStr.pt[self.n]  

            self.positionPP = pm.getParticleAttr('%s'%self.perParticle, at='position',a=True)
            self.getVertexNormal() 
            
    def multiJob_01(self):
        for self.n in self.jobNumDict[1]:
            self.perParticle = self.selectParticleStr.pt[self.n]  
    ...
    def multiJob_02(self):
        for self.n in self.jobNumDict[2]:
            self.perParticle = self.selectParticleStr.pt[self.n]  

            self.positionPP = pm.getParticleAttr('%s'%self.perParticle, at='position',a=True)
            self.getVertexNormal() 
            
    def multiJob_03(self):
        for self.n in self.jobNumDict[3]:
            self.perParticle = self.selectParticleStr.pt[self.n]  

            self.positionPP = pm.getParticleAttr('%s'%self.perParticle, at='position',a=True)
            self.getVertexNormal() 
            
    def multiJob_04(self):
        for self.n in self.jobNumDict[4]:
            self.perParticle = self.selectParticleStr.pt[self.n]  
