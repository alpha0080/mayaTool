...
class MyTreeItem(QtWidgets.QTreeWidgetItem,assetTreeInTheWorld):
...
class assetTreeInTheWorld(QtWidgets.QTreeWidget,mod_MainWindow):
    def __init__(self,parent= QtWidgets.QApplication.activeWindow()):
      ...
      #  MyTreeItem(self)
        #self.itemPressed.connect(self.itemPressNow)
        
   # def itemPressNow(self):
        
     #   MyTreeItem(self)
        
    def mousePressEvent(self, event):
        print 'tree mouse press'

        super(assetTreeInTheWorld, self).mousePressEvent(event)
        if event.button() == QtCore.Qt.LeftButton:
            self.mousePressPos = event.pos()    
        print self.mousePressPos     
       # self.currentItem().setFlags(QtCore.Qt.ItemIsSelectable|QtCore.Qt.ItemIsDropEnabled|QtCore.Qt.ItemIsUserCheckable|QtCore.Qt.ItemIsEnabled)
        self.currentItem().setFlags(QtCore.Qt.ItemIsDropEnabled)

    def mouseReleaseEven(self, event):
        print 'mouseReleaseEven'

        self.setAcceptDrop(True)
        event.accept()
        print 'tree mouse release'
