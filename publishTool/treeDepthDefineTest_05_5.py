...
class mod_MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def treeBuildPreItem(self,itemDictKeyIndex):
        
        print self.allAssetGrpDepthDict[self.allAssetGrpDepthDict.keys()[itemDictKeyIndex]]
        rangeCount = len(self.allAssetGrpDepthDict[self.allAssetGrpDepthDict.keys()[itemDictKeyIndex]]) # depth
      #  topLayerItem = QtWidgets.QTreeWidgetItem(self.treeWidget)
        #topLayerItemGet = self.treeWidget
       # topLayerItem = QtWidgets.QTreeWidgetItem(topLayerItemGet)
        #topLayerItem.setText(0,'tttt')
      ...
      #  print 
        ...
        maxCurrentLevelCount = int(self.allAssetGrpDepthDict[self.allAssetGrpDepthDict.keys()[itemDictKeyIndex]][0])
        topLayerItemIndex = int(self.allAssetGrpDepthDict[self.allAssetGrpDepthDict.keys()[itemDictKeyIndex]][0])
       ...
       # print 'currentTopLayerItemCount',currentTopLayerItemCount
      ...
      #  print delta
        ...
        for i in range(0,delta):   
          #  print i    
            ...
            #return topLayerItemIndex
            '''
            for j in range(0,maxCurrentLevelCount+1):
                print 'hahaha'
                item = self.treeWidget.topLevelItem(topLayerItemIndex)
             #   print item.text(0)
            '''
        item = self.treeWidget.topLevelItem(topLayerItemIndex)
      #  print 'item.childCount()',item.childCount()
        print 'itemName' ,item.text(0)
        
        #for i,j in zip(range(0,rangeCount+3),range(1,rangeCount)):  #how depth    ,rangeCount is the max depth for item
            #if item.childCount()
        for i in range(1,rangeCount):#how depth    ,rangeCount is the max depth for item
          #  print 'i',i,'j',j
            ...
            print 'childCount',self.childCount
            delta = (maxCurrentLevelCount)-self.childCount
            print 'childDelta',delta

            if delta >= 0:
                for j in range(0,delta):
                    self.createTreeItem(item,delta)
                    

            
            
    def createTreeItem(self,parentItem,itemCount):
