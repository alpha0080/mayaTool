...
class mod_MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    #def self.MODDEF(self):
        ...
        self.listWidget_gemSetting.clear()
        self.initialBuild()
...
#-----------------------------call module-------------------------        
    def click_button_Create(self):
        ...
        # create New RmasSet and build in List_widget
        rmanNewSetName = cmds.sets(n="rmanSet##")
        print cmds.nodeType(rmanNewSetName)
        

        
        
        itemCount =  self.listWidget_gemSetting.count()
        
        print itemCount
        if itemCount == 0:
            QtWidgets.QListWidgetItem(self.listWidget_gemSetting)
            self.listWidget_gemSetting.item(0).setText(QtWidgets.QApplication.translate("MainWindow", "tempName", None, -1))
            self.listWidget_gemSetting.item(0).setText('%s'%rmanNewSetName)
        else:
            print itemCount 
            #pass
            QtWidgets.QListWidgetItem(self.listWidget_gemSetting)
            self.listWidget_gemSetting.item(0).setText(QtWidgets.QApplication.translate("MainWindow", "tempName", None, -1))
            self.listWidget_gemSetting.item(itemCount).setText('%s'%rmanNewSetName)

           # rmanSetsList.append(i)
            
                

    def initialBuild(self):
        
        # check exist item in file
        rmanSetsList = []

        allRmanSets = cmds.ls(typ='objectSet')

        nodeExceptList = ['defaultLightSet','defaultObjectSet','initialParticleSE','initialShadingGroup']       
        for i in allRmanSets:
            if i in nodeExceptList:
                pass
            else:
                rmanSetsList.append(i)
        # build exist item in listWidget_gemSetting
        existItemCount =  len(rmanSetsList)
        
        for i in range(0,existItemCount):
            QtWidgets.QListWidgetItem(self.listWidget_gemSetting)
            self.listWidget_gemSetting.item(i).setText(QtWidgets.QApplication.translate("MainWindow", "tempName", None, -1))
            self.listWidget_gemSetting.item(i).setText('%s'%rmanSetsList[i])
